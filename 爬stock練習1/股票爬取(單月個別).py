...
def visualize(mm):
    ...
    show(p)
    #tab = Panel(child=column(p),title=mm)
   # return tab
...
#def total(pp):
    ...
    #show(pictures[0])
    #tab1 = Panel(child=column(pp[0]),title="0056")
    #tab2 = Panel(child=column(pp[1]),title="0050")
    #tab3 = Panel(child=column(pp[2]),title="2884")
    #tab4 = Panel(child=column(pp[3]),title="2891")
